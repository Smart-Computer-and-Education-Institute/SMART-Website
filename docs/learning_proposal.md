# Learning Proposal: Vercel + MongoDB Atlas Deployment Skill

## Classification

**Type**: Skill (multi-step actionable workflow with commands and configurations)

**Scope**: Project-specific (SMART-Website, Express + MongoDB + Vercel Blob stack)

**Location**: `.agents/skills/deploy-smart-website/SKILL.md` (workspace-level skill)

---

## Rationale

The user asked how to publish the SMART-Website to Vercel and connect it to a free MongoDB Atlas database. The project already has an excellent internal guide at [`docs/SECURITY_AND_DEPLOYMENT.md`](file:///m:/Git/SMART-Website/docs/SECURITY_AND_DEPLOYMENT.md), but it requires reading a long doc. A skill turns this into an agent-executable, step-by-step runbook that the agent can follow when the user says things like:

- *"deploy this to Vercel"*
- *"set up MongoDB Atlas for this project"*
- *"help me go live"*
- *"connect the database"*

No existing skill covers this workflow — this is a net-new creation.

---

## Proposed Skill Content

**File**: `m:\Git\SMART-Website\.agents\skills\deploy-smart-website\SKILL.md`
...
- [`api/index.js`](file:///m:/Git/SMART-Website/api/index.js) — Vercel serverless entry point
- [`vercel.json`](file:///m:/Git/SMART-Website/vercel.json) — rewrites all dynamic routes to the Express app
- [`lib/db.js`](file:///m:/Git/SMART-Website/lib/db.js) — auto-switches between MongoDB (production) and local JSON files (dev)
- [`lib/blobStorage.js`](file:///m:/Git/SMART-Website/lib/blobStorage.js) — auto-switches between Vercel Blob (production) and local `img/` (dev)
...
- **No existing skill is being modified** — this is a clean new workspace-level skill.
- The skill lives in `.agents/skills/` (workspace root) so it's auto-discovered by the agent for this project only.
- It references the existing [`docs/SECURITY_AND_DEPLOYMENT.md`](file:///m:/Git/SMART-Website/docs/SECURITY_AND_DEPLOYMENT.md) as its authoritative source.
