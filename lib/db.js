...
if (MONGODB_URI) {
  ...
  if (!global._smartMongoClientPromise) {
    // Initialise connection with error handling
    global._smartMongoClientPromise = (async () => {
      try {
        ...
        if (process.env.STRICT_MONGODB === "true") {
          console.error("❌ Exiting due to STRICT_MONGODB flag");
          process.exit(1);
        }
        // Leave mongoAvailable false; fallback to local JSON.
        return null;
      }
    })();
  }
...
async function getCollection(name) {
  if (!mongoAvailable) {
    throw new Error("MongoDB unavailable – falling back to local JSON storage");
  }
...
async function findAll(collectionName) {
  // Use local fallback if MongoDB URI not set **or** connection failed
  if (!MONGODB_URI || !mongoAvailable) {
