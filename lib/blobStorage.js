...
async function deleteUpload(storedPath) {
  ...
  const isOwned =
    storedPath.startsWith("img/gallery/") ||
    storedPath.startsWith("img/testimonials/");
  if (!isOwned) return;

  const folder = storedPath.startsWith("img/gallery/")
    ? "gallery"
    : "testimonials";
  const filename = path.basename(storedPath); // strips any directory part
  fs.unlink(path.join(__dirname, "..", "img", folder, filename), (err) => {
