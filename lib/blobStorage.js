...
async function saveUpload(buffer, folder, filename) {
  ...
  // from any page depth — including /admin/* pages, which would otherwise
  // resolve a bare "img/..." as "/admin/img/..." and 404.
  return `/img/${folder}/${filename}`;
}

// storedPath is whatever saveUpload() previously returned (a Vercel Blob URL,
// the new absolute "/img/..." path, or the legacy relative "img/..." path).
// Only ever deletes files this app created — never shared site assets.
async function deleteUpload(storedPath) {
  ...
  // so that records saved before this fix can still be cleaned up.
